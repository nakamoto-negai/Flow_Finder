...
type TouristSpotCategory struct {
	...
	Name         string         `gorm:"not null;unique" json:"name"`                                          // カテゴリ名
	...
	GroupID      *uint          `gorm:"index;constraint:OnUpdate:CASCADE,OnDelete:SET NULL;" json:"group_id"` // グループID
	Group        *CategoryGroup `gorm:"foreignKey:GroupID;references:ID" json:"group,omitempty"`              // グループ
